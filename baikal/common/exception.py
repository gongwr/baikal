    ...
    def __init__(self, message=None, *args, **kwargs):
        ...
        try:
            ...
            if _FATAL_EXCEPTION_FORMAT_ERRORS:
                ...
                pass
        self.msg = message
        super(BaikalException, self).__init__(message)


class MissingCredentialError(BaikalException):
    message = _("Missing required credential: %(required)s")


class BadAuthStrategy(BaikalException):
    message = _("Incorrect auth strategy, expected \"%(expected)s\" but "
                "received \"%(received)s\"")


class NotFound(BaikalException):
    message = _("An object with the specified identifier was not found.")


class Duplicate(BaikalException):
    message = _("An object with the same identifier already exists.")


class Conflict(BaikalException):
    message = _("An object with the same identifier is currently being "
                "operated on.")


class AuthBadRequest(BaikalException):
    message = _("Connect error/bad request to Auth service at URL %(url)s.")


class AuthUrlNotFound(BaikalException):
    message = _("Auth service at URL %(url)s not found.")


class AuthorizationFailure(BaikalException):
    message = _("Authorization failed.")


class NotAuthenticated(BaikalException):
    message = _("You are not authenticated.")


class Forbidden(BaikalException):
...
class AuthorizationRedirect(BaikalException):
    message = _("Redirecting to %(uri)s for authorization.")


class ClientConnectionError(BaikalException):
    message = _("There was an error connecting to a server")


class ClientConfigurationError(BaikalException):
    message = _("There was an error configuring the client.")


class MultipleChoices(BaikalException):
    message = _("The request returned a 302 Multiple Choices. This generally "
                "means that you have not included a version indicator in a "
                "request URI.\n\nThe body of response returned:\n%(body)s")


class LimitExceeded(BaikalException):
...
class ServiceUnavailable(BaikalException):
...
class ServerError(BaikalException):
    message = _("The request returned 500 Internal Server Error.")


class UnexpectedStatus(BaikalException):
    message = _("The request returned an unexpected status: %(status)s."
                "\n\nThe response body:\n%(body)s")


class InvalidContentType(BaikalException):
    message = _("Invalid content type %(content_type)s")


class BadRegistryConnectionConfiguration(BaikalException):
    message = _("Registry was not configured correctly on API server. "
                "Reason: %(reason)s")


class BadDriverConfiguration(BaikalException):
    message = _("Driver %(driver_name)s could not be configured correctly. "
                "Reason: %(reason)s")


class MaxRedirectsExceeded(BaikalException):
    message = _("Maximum redirects (%(redirects)s) was exceeded.")


class InvalidRedirect(BaikalException):
    message = _("Received invalid HTTP redirect.")


class NoServiceEndpoint(BaikalException):
    message = _("Response from Keystone does not contain a baikal endpoint.")


class WorkerCreationFailure(BaikalException):
    message = _("Server worker creation failed: %(reason)s.")


class SchemaLoadError(BaikalException):
    message = _("Unable to load schema: %(reason)s")


class InvalidObject(BaikalException):
    message = _("Provided object does not match schema "
                "'%(schema)s': %(reason)s")


class FailedToGetScrubberJobs(BaikalException):
    message = _("Scrubber encountered an error while trying to fetch "
                "scrub jobs.")


class SIGHUPInterrupt(BaikalException):
    message = _("System SIGHUP signal received.")


class RPCError(BaikalException):
    message = _("%(cls)s exception was raised in the last rpc call: %(val)s")


class TaskException(BaikalException):
    message = _("An unknown task exception occurred")


class BadTaskConfiguration(BaikalException):
...
class InvalidDataMigrationScript(BaikalException):
    message = _("Invalid data migration script '%(script)s'. A valid data "
                "migration script must implement functions 'has_migrations' "
                "and 'migrate'.")


class BaikalEndpointNotFound(NotFound):
    message = _("%(interface)s baikal endpoint not "
